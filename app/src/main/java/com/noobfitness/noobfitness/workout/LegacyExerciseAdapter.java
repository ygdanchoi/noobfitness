                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    ...
                    if (isChecked) {
                        ...
                        if (Integer.parseInt(counts[0]) + 1 == Integer.parseInt(counts[1])) {
                            animation = ObjectAnimator.ofObject(repsCounterCircle, "color", new ArgbEvaluator(), Color.argb(66, 0, 0, 0), ContextCompat.getColor(context, R.color.colorAccent));
                            animation.setDuration(225);
                            animation.setInterpolator(new DecelerateInterpolator());
                            animation.start();
                            repsCounterCircle.setColor(ContextCompat.getColor(context, R.color.colorAccent));
                        }
                    } else {
                        // update @id/reps_counter
                        repsCounter.setText((Integer.parseInt(counts[0]) - 1) + "/" + counts[1]);
